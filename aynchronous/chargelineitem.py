...
"""
chargelineitem-CHARGELINEITEMASSOC+SALESTRANLINEITEM
"""
...
COLUMN_MAPPING = {
    ...
    "SALESTRANDETAILID": ("SALESTRANDETAILID", "int8", "NUMBER"),
    "CHARGELINEITEMID": ("CHARGELINEITEMID", "int8", "NUMBER"),
    "SOURCENUMBER": ("SOURCENUMBER", "varchar(50)", "VARCHAR2(50)"),
    "OTLINK": ("OTLINK", "varchar(100)", None),
    "OTRID": ("CHARGENUMBER", "varchar(50)", "VARCHAR2(50)"),
    "OTSEQ": ("OTSEQ", "varchar(100)", None),
    "OTSSQ": ("OTSSQ", "varchar(100)", None),
    "OTCUE": ("OTCUE", "varchar(100)", None),
    "OTSHP": ("OTSHP", "varchar(100)", None),
    ...
    "CHARGENUMBER": ("CHARGENUMBER", "varchar(50)", "VARCHAR2(50)"),
    "CHARGETAXCODETYPE": ("CHARGETAXCODETYPE", "varchar(10)", "VARCHAR2(10)"),
    "CHARGETYPE": ("CHARGETYPE", "varchar(50)", "VARCHAR2(50)"),
    "CHARGECATEGORY": ("CHARGECATEGORY", "varchar(50)", "VARCHAR2(50)"),
    "CHARGEDESCRIPTION": ("CHARGEDESCRIPTION", "varchar(100)", "VARCHAR2(100)"),
    "CHARGEAMOUNT": ("CHARGEAMOUNT", "numeric(19, 6)", "NUMBER(19,6)"),
    "TAXPERCENTAGE": ("TAXPERCENTAGE", "numeric(10, 6)", "NUMBER(10,6)"),
    "TAXABLEIND": ("TAXABLEIND", "varchar(1)", "VARCHAR2(1)"),
    "POSTINGPRODUCTSKU": ("POSTINGPRODUCTSKU", "varchar(50)", "VARCHAR2(50)"),
...
# ========================== FETCH FUNCTIONS ==========================
def fetch_oracle_chargelineitem_data(source_numbers):
    ...
    try:
        ...
        sql = f"""
            SELECT 
                ...
                c.SALESTRANDETAILID,
                c.CHARGELINEITEMID,
                SUBSTR(c.SOURCEHEADERKEY, 1, INSTR(c.SOURCEHEADERKEY, '.') - 1) AS SOURCENUMBER,
                SUBSTR(c.SOURCEHEADERKEY, INSTR(c.SOURCEHEADERKEY, '.') + 1) AS OTLINK,
                c.CHARGENUMBER AS OTRID,
                sl1.SOURCESEQNUMBER AS OTSEQ,
                sl2.SOURCEDETAILID5 AS OTSSQ,
                sl3.SOURCEDETAILID4 AS OTCUE,
                sl4.SOURCEDETAILID3 AS OTSHP,
                c.SOURCEINSERTDATE,
                c.SOURCEINSERTUSERID,
                c.SOURCEUPDATEDATE,
                c.SOURCEUPDATEUSERID,
                c.INSERTDATE,
                c.UPDATEDATE,
                c.CHARGENUMBER,
                c.CHARGETAXCODETYPE,
                ...
                c.POSTINGPRODUCTSKU,
                c.SOURCESYSTEMID
            FROM CEX01_OWN.CHARGELINEITEMASSOC c
            LEFT JOIN CEX01_OWN.SALESTRANLINEITEM sl1 
                ON c.SALESTRANID = sl1.SALESTRANID 
                AND sl1.SOURCESYSTEMTABLE = 'SUNRISE_SFBASLIB_SFORDT_T'
            LEFT JOIN CEX01_OWN.SALESTRANLINEITEM sl2 
                ON c.SALESTRANID = sl2.SALESTRANID 
                AND sl2.SOURCESYSTEMTABLE = 'SUNRISE_SFBASLIB_SFORDT_T'
            LEFT JOIN CEX01_OWN.SALESTRANLINEITEM sl3 
                ON c.SALESTRANID = sl3.SALESTRANID 
                AND sl3.SOURCESYSTEMTABLE = 'SUNRISE_SFBASLIB_SFORDT_T'
            LEFT JOIN CEX01_OWN.SALESTRANLINEITEM sl4 
                ON c.SALESTRANID = sl4.SALESTRANID 
                AND sl4.SOURCESYSTEMTABLE = 'SUNRISE_SFBASLIB_SFORDT_T'
            WHERE SUBSTR(c.SOURCEHEADERKEY, 1, INSTR(c.SOURCEHEADERKEY, '.') - 1) IN ({placeholders})
        """
        cur.execute(sql)
...
def fetch_postgres_chargelineitem_data(source_numbers):
...
# ========================== MAIN RECONCILIATION ==========================
def reconcile_chargelineitem():
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    txt_log = f"reconcile_chargelineitem_{timestamp}.txt"
    json_report = f"report_chargelineitem_{timestamp}.json"

    with open(txt_log, "w", encoding="utf-8") as f, redirect_stdout(f):
        print("CHARGELINEITEM RECONCILIATION REPORT (AlloyDB ↔ On-Prem)")
        ...
        try:
            # Fetch Data
            ora_cols, ora_rows = fetch_oracle_chargelineitem_data(SOURCE_NUMBERS)
            pg_cols, pg_rows = fetch_postgres_chargelineitem_data(SOURCE_NUMBERS)
