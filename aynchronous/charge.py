...
COLUMN_MAPPING = {
    ...
    "CHARGEID": ("SALESTRANCHARGEID", "int8", "NUMBER"),
    "SOURCENUMBER": ("SOURCENUMBER", "varchar(50)", "VARCHAR2(50)"),
    "OHLINK": ("OHLINK", "varchar(100)", None),
...
def reconcile_charge():
    ...
    txt_log = f"reconcile_charge_{timestamp}.txt"
    json_report = f"report_charge_{timestamp}.json"

    with open(txt_log, "w", encoding="utf-8") as f, redirect_stdout(f):
        print("CHARGE RECONCILIATION REPORT (AlloyDB ↔ On-Prem)")
        ...
        try:
            ...
            for sn in sorted(common_sns):
                ...
                # Compare all combinations (multiple rows per source)
